...
df = pd.read_csv('f:/dataset/sentiment/experiment.txt', delimiter="\t")
...
def home_screen():
    ...
    btn_login = Button(frm, command=lambda: welcome_screen(entry_user, entry_pass), text="login", font=('', 20, 'bold'),
                       bd=10, width=10)
    btn_login.place(relx=.45, rely=.5)


def welcome_screen(entry_user=None, entry_pass=None):
    if (entry_pass != None and entry_user != None):
        user = entry_user.get()
        pwd = entry_pass.get()
    else:
        user = "admin"
    ...
    if (len(user) == 0 or len(pwd) == 0):
        ...
        if (user == "admin" or pwd == "admin"):
            ...
            btn_single = Button(frm, command=lambda: single_feedback_screen(), text="Single Feedback Prediction",
                                font=('', 20, 'bold'), bd=10, width=25)
            btn_single.place(relx=.35, rely=.2)

            btn_bulk = Button(frm, command=lambda: bulk_feedback_screen(), text="Bulk Feedback Prediction",
                              font=('', 20, 'bold'), bd=10, width=25)
...
def single_feedback_screen():
    ...
    # entry_pass=Entry(frm,font=('',20,'bold'),bd=10,show="*")
    # entry_pass.place(relx=.35,rely=.55)

    btn_login = Button(frm, command=lambda: predict_single(entry_user, lbl_result), text="predict",
                       font=('', 20, 'bold'), bd=10, width=8)
...
def bulk_feedback_screen():
    ...
    btn_login = Button(frm, text="predict and save", font=('', 20, 'bold'), bd=10, width=15)
